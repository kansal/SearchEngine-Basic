...
#define MAX_TERMS 15 //max terms to print
#define MAX_DOC_NUM 300000 //max no of documnets
#define MULTI_MATCH_BOOST 2.0 //if doc contains all the terms of query, more weight
#define MIN_WORD_COUNT 1 // To prevent short document normalization issues
#define _FILE_OFFSET_BITS 64
#define ulli unsigned long long int //offset_t is 32 bit some architecture. This forcefully made 64 bit.
ulli doccnt=15000000; /* total number of doc in corpus*/
ulli docWordCount=1; /* get the number of word in doc */
ulli countField[7]={1,1,1,1,1,1,6}; //it "t" search word-t
...
bool my_function (string i,string j){ //no use
    return (strtoul(i.c_str(),NULL, 0) < strtoul(j.c_str(),NULL, 0));
}

bool my_func(pair < string, off_t > a,pair < string, off_t > b){ //no use
...
//split string based on the delim d
...
// give doc ide gives op as its no of words. 
...
//takes doc id gove title // this and above functoins are approx same.
...
//takes string and converts to lower and removes garbage like *(),
...
//give token, returns docid and (tf*idf/lengthOfDoc)
...
//scans input and send to proecss_token
